...
public class MyHttpServer {
    ...
    public static class MyHandler implements HttpHandler {
            ...
            if (uri.endsWith(".gif") || uri.endsWith(".ico")) {
                ...
                if (requestCookie == null || requestCookie.equals("0") || cookie.equals("0") || !requestCookie.equals(cookie)) {
                    ...
                    if (validateInput(uri)) {
                        int result = game.playGame(uri.charAt(uri.length() - 1));
        ...
        private boolean validateInput(String uriRequest) {
            String uri = uriRequest.substring(uriRequest.indexOf('=') + 1);

            if (uri.isEmpty() || uri.length() > 1) {
                return false;
            } else {
                char choice = uri.charAt(0);
